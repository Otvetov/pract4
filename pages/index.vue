...
<template>
    ...
    <div class="col" v-for="film in filmStore.films" :key="film.id">
      <div class="card h-100">
        ...
        <div class="cart-footer">
          <button @click="$router.push('/film/'+film.id)" class="btn btn-outline-primary mt-3 " type="submit">смотреть</button>
        </div>
      </div>
    </div>
  ...
  <nav v-if="!filmStore.isLoading" class="d-flex justify-content-center mt-4" aria-label="Page navigation example">
    <ul class="pagination">
      <li
          :class="{'disabled': filmStore.currentPage === 1}"
          class="page-item">
        <a @click.prevent="goto(filmStore.currentPage - 1)"
            class="page-link"
